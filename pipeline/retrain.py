...
def build_loaders(data_dir, batch_size, workers=0):
...
def run_phase(model, train_loader, val_loader, train_ds, criterion,
              optimizer, device, epochs, patience, phase_name):
    """Train for up to `epochs`, early-stop on val loss, return best state."""
...
def main():
    ...
    ap.add_argument("--task", required=True,
                    help="name used in output filename, e.g. silting, silting_binary, corrosion")
    ap.add_argument("--epochs", type=int, default=40)
    ap.add_argument("--batch-size", type=int, default=32)
    ap.add_argument("--lr", type=float, default=1e-3)
    ap.add_argument("--unfreeze", action="store_true",
                    help="two-phase: train head frozen, then fine-tune whole backbone (GPU)")
    ap.add_argument("--finetune-lr", type=float, default=1e-5,
                    help="low LR for the unfreeze phase (default 1e-5)")
    ap.add_argument("--finetune-epochs", type=int, default=30,
                    help="max epochs for the unfreeze phase")
    ap.add_argument("--patience", type=int, default=7,
                    help="early-stopping patience on val loss")
    ap.add_argument("--tag", default="v2",
                    help="version tag in output filename (e.g. 'field', 'v3')")
    ap.add_argument("--workers", type=int, default=0,
                    help="DataLoader workers (set 4-8 on GPU machines for speed)")
    args = ap.parse_args()
    ...
    train_ds, train_loader, val_loader, test_loader = build_loaders(
        args.data, args.batch_size, workers=args.workers)
    num_classes = len(train_ds.classes)
    print(f"Task: {args.task} | classes (folder order): {train_ds.classes}")
    print(f"Train: {len(train_ds)} images")

    # always start with frozen backbone (head warmup)
    model = CulvertClassifier(num_classes, freeze_backbone=True).to(device)
    weights = class_weights(train_ds, device)
    print(f"Class weights (imbalance correction): {weights.tolist()}")
    criterion = nn.CrossEntropyLoss(weight=weights)

    # Phase 1: train the custom head only
    head_params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.Adam(head_params, lr=args.lr)
    best_state = run_phase(model, train_loader, val_loader, train_ds, criterion,
                           optimizer, device, args.epochs, args.patience,
                           "PHASE 1: head training (backbone frozen)")
    if best_state:
        model.load_state_dict(best_state)

    # Phase 2 (optional): unfreeze backbone, fine-tune everything at low LR
    if args.unfreeze:
        for p in model.backbone.features.parameters():
            p.requires_grad = True
        all_params = [p for p in model.parameters() if p.requires_grad]
        optimizer = torch.optim.Adam(all_params, lr=args.finetune_lr)
        best_state = run_phase(model, train_loader, val_loader, train_ds, criterion,
                               optimizer, device, args.finetune_epochs, args.patience,
                               f"PHASE 2: full fine-tune (lr={args.finetune_lr})")
        if best_state:
            model.load_state_dict(best_state)
    ...
    out_pt = os.path.join(models_dir, f"{args.task}_model_{args.tag}_{stamp}.pt")
    scripted.save(out_pt)

    classmap = {i: train_ds.classes[i] for i in range(num_classes)}
    out_json = os.path.join(models_dir, f"{args.task}_classmap_{args.tag}_{stamp}.json")
